...
data['PC3'] = reduced_features[:, 2]
data['PC4'] = reduced_features[:, 3]
...
stratified_sample = data.groupby(['PC1_quantile', 'PC2_quantile', 'PC3_quantile', 'PC4_quantile'], group_keys=False).apply(
...
kmeans.fit(stratified_sample[['PC1', 'PC2', 'PC3', 'PC4']])
...
final_sample = stratified_sample.groupby('cluster_label', group_keys=False).apply(
    lambda x: x.iloc[((x[['PC1', 'PC2', 'PC3', 'PC4']] - kmeans.cluster_centers_[x['cluster_label'].iloc[0], :])**2).sum(axis=1).idxmin()]  
...
# 保存结果
final_sample.to_csv('Kmeans_STD_mod_200_layer_mod_1.csv', index=False)